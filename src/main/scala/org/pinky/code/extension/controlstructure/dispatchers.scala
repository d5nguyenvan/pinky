...
class ActorHolder[T](m: Manifest[T]) {
  ...
  private def createInstanceOfActorClientAndLaunchItWith(data: Map[String, AnyRef],actors:Actor*) = {
    try {
      ...
      constructor.newInstance(Seq(data)++actors.toSeq).asInstanceOf[ActorClient]
    } catch {
      case e:Exception=> {
        println("error creating Actor client. Data:"+data.toString+" type:"+m.erasure.toString)
        e.printStackTrace
      }
    }
  }
...
abstract class ActorClient(reqData: Map[String, AnyRef],actors:Actor*) extends Actor{
  ...
  val delay:String = (this.getClass().getAnnotation(classOf[Every])).asInstanceOf[Every].value()
  def fireStarter(block: => Unit): Unit = {
    if (delay != null) {
      executor.scheduleWithFixedDelay(new java.lang.Runnable {def run = block}, parseDuration(delay), parseDuration(delay), TimeUnit.SECONDS)
    } else block
  }
